...
const QUOTES: ReactNode[] = [
  <MarqueeQuote key="marquee">
    I still mourn the death of the <Code>&lt;marquee&gt;</Code> tag.
  </MarqueeQuote>,
  <>The mountains never judged my code. The ocean doesn’t either.</>,
  <>
...
function pickDifferentQuote(from: number): number {
  if (QUOTES.length <= 1) return from;
  let next = from;
  while (next === from) next = Math.floor(Math.random() * QUOTES.length);
  return next;
}

// The first quote has to be picked on the client — randomising during SSR would
// mismatch on hydration. Cached, so the snapshot stays stable between reads.
let seededIndex: number | null = null;
const subscribeToNothing = () => () => {};
const getSeededIndex = () =>
  (seededIndex ??= Math.floor(Math.random() * QUOTES.length));
const getServerIndex = () => MARQUEE_INDEX;
...
export default function QuoteContent() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setChosen(pickDifferentQuote(index)), delay);
